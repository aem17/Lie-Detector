...
def mouth_aspect_ratio(mouth):
    mA = dist.euclidean(mouth[1], mouth[11])
    mB = dist.euclidean(mouth[2], mouth[10])
    mC = dist.euclidean(mouth[3], mouth[9])
    mD = dist.euclidean(mouth[4], mouth[8])
    mE = dist.euclidean(mouth[5], mouth[7])
    mF = dist.euclidean(mouth[0], mouth[6])

    mar = (mA + mB + mC + mD + mE) / (5.0 * mF)
    return mar
...
while True:
    ...
    for rect in rects:
        ...
        if mar < MOUTH_AR_THRESH:
            mouthCOUNTER += 1
        # elif (mar < MOUTH_AR_THRESH) and (mouthCOUNTER <= MOUTH_AR_CONSEC_FRAMES):
